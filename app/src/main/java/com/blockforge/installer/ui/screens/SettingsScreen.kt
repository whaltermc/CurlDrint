...
fun SettingsScreen(viewModel: BrowseViewModel, onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(Modifier.padding(padding).fillMaxSize().padding(16.dp)) {
            ...
            Text(
                "About installing to Amethyst launcher",
                style = MaterialTheme.typography.titleMedium,
                color = MaterialTheme.colorScheme.onSurface
            )
            Spacer(Modifier.height(4.dp))
            Text(
                "When you tap Install, Android's folder picker (Storage Access Framework) opens " +
                    "so you can navigate to and select Amethyst's game data folder for that content " +
                    "type (e.g. its mods, resource_packs, worlds or shader_packs folder). The app " +
                    "remembers your last choice per category so future installs are one tap.",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
        }
